...
const connectionString = process.env.PG_LOCAL_STRING; //from online database
...
export async function insertPost(postData) {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO post(heading,content,description,status,image) VALUES($1, $2, $3,$4,$5)`,
      [
        postData.heading,
        postData.content,
...
export async function updatePost(oldHeading, newHeading, description, content,image) {
  try {
    content = content.trim();
    let singlePost = await client.query(`update post set heading='${newHeading}', content = '${content}', description ='${description}',image='${image}',updated_at = now() where heading='${oldHeading}';
    `);
...
export async function getSelectedPost(heading) {
  try {
    let post = await client.query(`select heading,content,description,image from post where heading = '${heading}';
    `);
