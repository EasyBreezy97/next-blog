...
export async function deletePost(heading) {
  ...
  console.log('heading from deletepost:',heading, typeof heading)
  try {
    await client.query(
...
export async function getPostHeadings() {
  try {
    let headings = await client.query(`select heading from post where status <> 0 order by heading asc;`);
    console.log("rOWSSS::::", headings.rows);
...
export async function hasHeading(heading) {
  try {
    ...
    return headingsArray.filter(element => element.heading === heading).length !== 0;
